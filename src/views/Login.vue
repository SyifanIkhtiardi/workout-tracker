<template>
  <div class="max-w-screen-sm mx-auto px-4 py-10">
    ...
    <form @submit.prevent="login" class="p-8 flex flex-col bg-light-grey rounded-md shadow-lg">
      ...
      <button type="submit" class="mt-6 py-2 px-6 rounded-sm self-start text-sm text-white bg-at-light-green duration-200 border-solid border-2 border-transparent hover:border-at-light-green hover:bg-white hover:text-at-light-green">Login
      </button>

      <router-link class="text-sm mt-6 text-center" :to="{ name:'Register' }">
        Dont't have account? <span class="text-at-light-green">Register Now</span>
      </router-link>
    </form>
  </div>
</template>
